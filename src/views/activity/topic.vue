...
<script>
import { listTopic, deleteTopic } from '@/api/activity/topic'
...
import Pagination from '@/components/Pagination' // Secondary package based on el-pagination
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push({ path: '/activity/topic-create' })
    },
    handleUpdate(row) {
      this.$router.push({ path: '/activity/topic-edit', query: { id: row.id }})
    },
    handleDelete(row) {
      deleteTopic(row)
        .then(response => {
